...
private void readHTTPRequest(InputStream is)
{
   ...
   // find the file path to read
   String a[] = new String [3];
   a = line.split(" ");
   filePath = a[1];
   
   // find out the type of file we are reading
...
private void writeContent(OutputStream os) throws Exception
{
   ...
   if (fileType.equalsIgnoreCase("png")) {
      ...
      // create a byte array to store all the bytes from the image file
      byte b[] = null;
      
      // used to find the length of the file
      File c = new File(filePath);
      
      // make the byte array as big as the file length
      b = new byte [(int)c.length()];
      
      // read all of the bytes in the file and load them into the byte array
      a.read(b);
      // close the file
      a.close();
